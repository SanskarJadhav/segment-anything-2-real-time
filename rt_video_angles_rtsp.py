...
class SamImagePublisher(Node):
    def __init__(self):
        ...
        self.cap, frame = setup_video("rtsp://192.168.0.200:8554/stream")
...
#     def __init__(self):
#         super().__init__('sam_image_publisher')
#         self.publisher = self.create_publisher(Image, 'sam_image', 10)
#         self.bridge = CvBridge()
#         setup_cuda()
#         self.predictor = setup_sam2(
...
#                 # Compute the longest edge angle
#                 contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
#                 max_length = 0
#                 best_angle = None
#                 for contour in contours:
...
#                 # Resize the overlay back to original size
#                 overlay = cv2.resize(overlay, self.original_size, interpolation=cv2.INTER_LINEAR)
                
#                 # Draw angle text clearly on the resized frame
#                 if best_angle is not None:
#                     cv2.putText(overlay, f"Angle: {best_angle:.1f}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (20, 255, 20), 2, cv2.LINE_AA)
