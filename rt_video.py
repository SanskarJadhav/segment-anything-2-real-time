"""
Real-time video segmentation using SAM2 (Segment Anything Model 2).
Supports both point-based and bounding box-based prompting for object segmentation.
"""
...
def process_frame(frame: np.ndarray, mask_logits: torch.Tensor, fps: float = None) -> np.ndarray:
    ...
    for i in range(3):
        overlay_frame[:,:,i] = np.where(mask == 1, overlay_color[i], overlay_frame[:,:,i])
    ...
    #         overlay_frame,
    #         fps_text,
    #         (overlay_frame.shape[1] - 120, 30),  # position
...
def main():
    ...
    while True:
        ...
        if frame_idx%fin_obj_id == 1:
            ...
            # fps = 1 / (current_time - prev_time) if (current_time - prev_time) > 0 else fps
            # fps = alpha * fps + (1 - alpha) * fps
            ...
            #     overlay,
            #     fps_text,
            #     (overlay.shape[1] - 120, 30),  # position
            #     cv2.FONT_HERSHEY_SIMPLEX,
            #     0.5,  # font scale
            #     (255, 255, 255),
            #     2,  # thickness
